...
function run() {
		...
		return new Promise(function(resolve, reject) {
			if (!_this.dt || !!_this.dt.pricescraper || !!_this.dt.priceScraper) {
				debug('_this.dt.priceScraper is true so it wont spawn again');
				return resolve(true);
			}
			_this.getCache()
				.then(function(cache) {
					...
					return _this.getAll()
						.then(function(results) {
							...
							_this.saveCache(prices, function(err, res) {
								if (err) {
									debug('Error on saveCache: ' + err.message);
									return reject(err);
								}
								return resolve(prices);
							});
